...
- Agent 在调用 `ui_ux_search` / `generate_design_system` / `persist_design_system` 等工具并完成**实现代码生成**（由模型写出 HTML 或 React 源码，仍通过现有 [`write_review_file`](../../tools/storage/write_review_file.go) / [`tempstorage`](../../tempstorage/tempstorage.go) 落盘）后，**宿主前端能预览**。
